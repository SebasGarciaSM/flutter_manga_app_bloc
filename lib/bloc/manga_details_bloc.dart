...
class MangaDetailsBloc extends Disposable {
  ...
  StreamSink<ApiResponse<MangaDetails>> get mangaDetailsSink =>
      _mangaDetailsController.sink;
  Stream<ApiResponse<MangaDetails>> get mangaDetailsStreams =>
      _mangaDetailsController.stream.asBroadcastStream();
  ...
  fetchMangaDetails(Manga manga) async {
    mangaDetailsSink
        .add(ApiResponse.loading('Fetching ${manga.title} details'));
    try {
      MangaDetails mangaDetails =
          await _mangaTownRepository.fetchMangaDetails(manga);
